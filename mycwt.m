...
% Continuous Wavelet Transform =========
CWT = zeros(length(t),length(scale));
...
%parpool('local',2);
% parfor s = 1:length(scale) % for each scale (maximum N)
for s = 1:length(scale) % for each scale (maximum N)   
    
    Ws0 = zeros(1,N); % Morlet mother wavelet
    Ws = zeros(N,N);
...
%    psi_hat = zeros(N,N); 
%    psi_hat = F'.*(conj(Ws0));
%    [ CWT(:,s) tscale ] = myidft( psi_hat, omega );
